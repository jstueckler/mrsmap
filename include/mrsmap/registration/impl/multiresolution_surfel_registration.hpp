...
inline typename TMRSMap::Surfel* bestSurfelMatch( spatialaggregate::OcTreeNode<float, typename TMRSMap::NodeValue>* node,
...
template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::Params::Params() {
	init();
}

template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::Params::init() {
...
template <typename TMRSMap>
std::string MultiResolutionSurfelRegistration<TMRSMap>::Params::toString() {
...
template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::MultiResolutionSurfelRegistration() {

}


template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::MultiResolutionSurfelRegistration( const Params& params ) {
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::setPriorPose( bool enabled, const Eigen::Matrix< double, 6, 1 >& prior_pose_mean, const Eigen::Matrix< double, 6, 1 >& prior_pose_variances ) {
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::setPriorPose( bool enabled, const Eigen::Matrix< double, 6, 1 >& prior_pose_mean, const Eigen::Matrix< double, 6, 6 >& prior_pose_cov ) {
...
template <typename TMRSMap>
spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* MultiResolutionSurfelRegistration<TMRSMap>::getOccluder2( const Eigen::Vector4f& p, const TMRSMap& target, double z_similarity_factor ) {
	...
	if( !target.imageAllocator_->node_set_.empty() ) {
		...
		spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n = target.imageAllocator_->node_image_[idx];
		if( n ) {
...
template <typename TMRSMap>
spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* MultiResolutionSurfelRegistration<TMRSMap>::getOccluder( spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* node, const TMRSMap& target, const Eigen::Matrix4d& transform ) {
...
//	const double normalStd = 0.25*M_PI;
	...
	Eigen::Matrix3d rotation = transform.template block<3,3>(0,0);
...
//		return NULL;
	...
	std::vector< spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* > neighbors;
	...
	spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n_matched = NULL;
	typename TMRSMap::Surfel* srcSurfel = NULL;
	typename TMRSMap::Surfel* matchedSurfel = NULL;
	int matchedSurfelIdx = -1;
	double bestDist = std::numeric_limits<double>::max();

	// get closest node in neighbor list
	for( unsigned int i = 0; i < TMRSMap::NodeValue::num_surfels_; i++ ) {

		typename TMRSMap::Surfel& surfel = node->value_.surfels_[i];
...
//		}
		...
		Eigen::Vector4d pos;
		pos.template block<3,1>(0,0) = surfel.mean_.template block<3,1>(0,0);
		pos(3,0) = 1.f;

		Eigen::Vector4d pos_match_src = transform * pos;
		Eigen::Vector3d dir_match_src = rotation * surfel.initial_view_dir_;

		for( typename std::vector< spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* >::iterator it = neighbors.begin(); it != neighbors.end(); it++ ) {

			if( (*it)->value_.border_ != node->value_.border_ )
				continue;

			typename TMRSMap::Surfel* bestMatchSurfel = NULL;
			int bestMatchSurfelIdx = -1;
			double bestMatchDist = -1.f;
			for( unsigned int k = 0; k < TMRSMap::NodeValue::num_surfels_; k++ ) {

				const typename TMRSMap::Surfel& srcSurfel2 = (*it)->value_.surfels_[k];
			...
			if( bestMatchSurfel ) {
				...
				double dist = (pos_match_src.template block<3,1>(0,0) - bestMatchSurfel->mean_.template block<3,1>(0,0)).norm();
				if( dist < bestDist ) {
					bestDist = dist;
					srcSurfel = &surfel;
					n_matched = *it;
...
//		return NULL;
	...
	Eigen::Vector4d pos;
	pos.template block<3,1>(0,0) = srcSurfel->mean_.template block<3,1>(0,0);
	pos(3,0) = 1.f;
...
//		rotation6.template block<3,3>(0,0) = rotation;
		...
		if( interpolate ) {
			...
			for( int s = 0; s < 27; s++ ) {
				...
				spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n_dst_n = n_matched->neighbors_[s];

				if(!n_dst_n)
					continue;

				typename TMRSMap::Surfel* dst_n = &n_dst_n->value_.surfels_[matchedSurfelIdx];
				if( dst_n->num_points_ < MIN_SURFEL_POINTS )
					continue;

				Eigen::Vector3d centerDiff_n = n_dst_n->getCenterPosition().template block<3,1>(0,0).template cast<double>() - pos_match_src.template block<3,1>(0,0);
...
//		const Eigen::Matrix< double, 6, 6 > cov2 = params_.interpolation_cov_factor_ * srcSurfel->cov_;
		...
		diff.template block<3,1>(0,0) = dstMean.template block<3,1>(0,0) - pos_match_src.template block<3,1>(0,0);
		diff.template block<3,1>(3,0) = dstMean.template block<3,1>(3,0) - srcSurfel->mean_.template block<3,1>(3,0);
		...
		// has only marginal (positive!) effect on visual odometry result
		// makes tracking more robust (when only few surfels available)
		const Eigen::Matrix3d cov1_ss = cov1.template block<3,3>(0,0);
		const Eigen::Matrix3d cov2_ss = cov2.template block<3,3>(0,0);
...
//		const Eigen::Matrix3d cov_ss = node->resolution()*node->resolution() * Eigen::Matrix3d::Identity();
		...
		const Eigen::Vector3d invcov_ss_diff_s = invcov_ss * diff.template block<3,1>(0,0);

//		l = log( cov_ss.determinant() ) + diff.template block<3,1>(0,0).dot(invcov_ss_diff_s);
		l = diff.template block<3,1>(0,0).dot(invcov_ss_diff_s);
//		l = std::max( 0.0, l - 9.0 );
//		if( l < 9 )
//			l = 0;
		if( l > 48.0 )
...
//		std::cout << "s:\n";
//		std::cout << diff.template block<3,1>(0,0) << "\n";
//		std::cout << cov_ss.template block<3,3>(0,0) << "\n";
//		std::cout << log( cov_ss.determinant() ) << "\n";
//		std::cout << l << "\n";

		const Eigen::Matrix3d cov_cc = cov1.template block<3,3>(3,3) + cov2.template block<3,3>(3,3) + color_z_cov * Eigen::Matrix3d::Identity();
//		l += log( cov_cc.determinant() ) + diff.template block<3,1>(3,0).dot( cov_cc.inverse() * diff.template block<3,1>(3,0) );
		double color_loglikelihood = diff.template block<3,1>(3,0).dot( (cov_cc.inverse() * diff.template block<3,1>(3,0)).eval() );
		...
		if( interpolate ) {
			...
			for( int s = 0; s < 27; s++ ) {
				...
				if( dx > 0 && dy > 0 && dz > 0 ) {
					...
					dstMean += weight * dst_n->mean_.template block<3,1>(0,0);
					cov1_ss += weight*weight * (dst_n->cov_.template block<3,3>(0,0));
		...
		if( !interpolate || !in_interpolation_range ) {
			...
			dstMean = matchedSurfel->mean_.template block<3,1>(0,0);
			cov1_ss = matchedSurfel->cov_.template block<3,3>(0,0);
		...
		const Eigen::Matrix3d cov2_ss = params_.interpolation_cov_factor_ * srcSurfel->cov_.template block<3,3>(0,0);

		const Eigen::Vector3d diff_s = dstMean - pos_match_src.template block<3,1>(0,0);
...
//	if( srcSurfel->surface_curvature_ < 0.05 ) {
		...
		normal_src.template block<3,1>(0,0) = srcSurfel->normal_;
		normal_src(3,0) = 0.0;
		normal_src = (transform * normal_src).eval();

		double normalError = acos( normal_src.template block<3,1>(0,0).dot( matchedSurfel->normal_ ) );
...
template <typename TMRSMap>
spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* MultiResolutionSurfelRegistration<TMRSMap>::calculateNegLogLikelihoodN( double& logLikelihood, bool& outOfImage, bool& virtualBorder, bool& occluded, spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* node, const TMRSMap& target, const Eigen::Matrix4d& transform, double spatial_z_cov_factor, double color_z_cov, double normal_z_cov, bool interpolate ) {
...
template <typename TMRSMap>
bool MultiResolutionSurfelRegistration<TMRSMap>::calculateNegLogLikelihood( double& logLikelihood, spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* node, const TMRSMap& target, const Eigen::Matrix4d& transform, double spatial_z_cov_factor, double color_z_cov, double normal_z_cov, bool interpolate ) {
...
template <typename TMRSMap>
double MultiResolutionSurfelRegistration<TMRSMap>::calculateInPlaneLogLikelihood( spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n_src, spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n_tgt, const Eigen::Matrix4d& transform, double normal_z_cov ) {

	double bestLogLikelihood = 18.0;
	for( unsigned int i = 0; i < TMRSMap::NodeValue::num_surfels_; i++ ) {

		typename TMRSMap::Surfel& s_src = n_src->value_.surfels_[i];
		typename TMRSMap::Surfel& s_tgt = n_tgt->value_.surfels_[i];
		...
		const Eigen::Vector3d mean_src = s_src.mean_.template block<3,1>(0,0);
		const Eigen::Vector3d mean_tgt = s_tgt.mean_.template block<3,1>(0,0);
		const Eigen::Vector4d mean_src4( mean_src(0), mean_src(1), mean_src(2), 1.0 );
		const Eigen::Vector3d mean_src_transformed = (transform * mean_src4).template block<3,1>(0,0);

		const Eigen::Matrix3d rot_src = transform.template block<3,3>(0,0);
		const Eigen::Matrix3d cov_src_transformed = rot_src * (s_src.cov_.template block<3,3>(0,0)) * rot_src.transpose();
		const Eigen::Matrix3d cov_tgt = s_tgt.cov_.template block<3,3>(0,0);
		...
		Eigen::Vector4d normal_src;
		normal_src.template block<3,1>(0,0) = s_src.normal_;
		normal_src(3,0) = 0.0;
		normal_src = (transform * normal_src).eval();

		double normalError = acos( normal_src.template block<3,1>(0,0).dot( s_tgt.normal_ ) );
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::associateMapsBreadthFirstParallel( MultiResolutionSurfelRegistration<TMRSMap>::SurfelAssociationList& surfelAssociations, TMRSMap& source, TMRSMap& target, algorithm::OcTreeSamplingVectorMap< float, typename TMRSMap::NodeValue >& targetSamplingMap, Eigen::Matrix4d& transform, double minResolution, double maxResolution, double searchDistFactor, double maxSearchDist, bool useFeatures ) {
...
template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::AssociateFunctor::~AssociateFunctor() {

}


template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::AssociateFunctor::operator()( const tbb::blocked_range<size_t>& r ) const {
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::AssociateFunctor::operator()( spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >*& node ) const {

		spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n = node;
...
//			}
		...
		if( hasSurfel ) {
			...
			spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n_src_last = NULL;
			std::vector< spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* > neighbors;
			...
			if( surfelSrcIdx >= 0 && surfelDstIdx >= 0 ) {
				...
				const typename TMRSMap::Surfel& surfel = n->value_.surfels_[surfelSrcIdx];
				...
				if( surfel.num_points_ >= MIN_SURFEL_POINTS ) {
					...
					for( typename std::vector< spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* >::iterator nit = neighbors.begin(); nit != neighbors.end(); ++nit ) {

						spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n_src = *nit;
						...
						typename TMRSMap::Surfel& dstSurfel = n_src->value_.surfels_[surfelDstIdx];
						...
						double dist_pos2 = (bestMatchSurfel->mean_.template block<3,1>(0,0) - pos_match_src.template block<3,1>(0,0)).squaredNorm();
						...
						if( params_.registration_use_color_ ) {
							...
							diff = bestMatchSurfel->mean_.template block<3,1>(3,0) - surfel.mean_.template block<3,1>(3,0);
						...
						float assocDist = sqrtf(dist_pos2);// + process_resolution_*process_resolution_*(bestMatchSurfel->mean_.template block<3,1>(3,0) - surfel.mean_.template block<3,1>(3,0)).squaredNorm());
...
//								bestAssoc = SurfelAssociation( n, &n->value_.surfels_[surfelSrcIdx], surfelSrcIdx, n_src, bestMatchSurfel, bestMatchSurfelIdx );
							bestAssoc.n_src_ = n;
				...
				for( unsigned int i = 0; i < TMRSMap::NodeValue::num_surfels_; i++ ) {
					...
					const typename TMRSMap::Surfel& surfel = n->value_.surfels_[i];
...
//					}
					...
					for( typename std::vector< spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* >::iterator nit = neighbors.begin(); nit != neighbors.end(); ++nit ) {
						...
						for( unsigned int k = 0; k < TMRSMap::NodeValue::num_surfels_; k++ ) {
							...
							const typename TMRSMap::Surfel& srcSurfel = n_src->value_.surfels_[k];
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::associateNodeListParallel( SurfelAssociationList& surfelAssociations, TMRSMap& source, TMRSMap& target, std::vector< spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* >& nodes, int processDepth, Eigen::Matrix4d& transform, double searchDistFactor, double maxSearchDist, bool useFeatures ) {

	tbb::concurrent_vector< SurfelAssociation > depthAssociations;
	depthAssociations.reserve( nodes.size() );

	// only process nodes that are active (should improve parallel processing)
	std::vector< spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* > activeNodes;
	activeNodes.reserve( nodes.size() );

	for( unsigned int i = 0; i < nodes.size(); i++ ) {

		spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n = nodes[i];
...
template <typename TMRSMap>
typename MultiResolutionSurfelRegistration<TMRSMap>::SurfelAssociationList MultiResolutionSurfelRegistration<TMRSMap>::revertSurfelAssociations( const SurfelAssociationList& surfelAssociations ) {

	SurfelAssociationList retVal = surfelAssociations;
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::associatePointFeatures() {
	...
	for (unsigned int i = 0; i < sourceQuery.rows; i++) {
		...
		for (unsigned int n = 0; n < numNeighbors; n++) {
			...
			for (unsigned int n2 = 0; n2 < numNeighbors; n2++) {
				...
				if (sourceIdx == i) {
					FeatureAssociation assoc( i, targetIdx);
...
template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::GradientFunctor::GradientFunctor( SurfelAssociationList* assocList, const Params& params, double tx, double ty, double tz, double qx, double qy, double qz, double qw, bool relativeDerivatives, bool deriv2, bool interpolate_neighbors, bool derivZ ) {
		...
		currentTransform.template block<3,3>(0,0) = Eigen::Matrix3d( Eigen::Quaterniond( qw, qx, qy, qz ) );
...
//		cov_cc_add *= SMOOTH_COLOR_COVARIANCE;
		...
		currentRotation = Eigen::Matrix3d( currentTransform.template block<3,3>(0,0) );
		currentRotationT = currentRotation.transpose();
		currentTranslation = Eigen::Vector3d( currentTransform.template block<3,1>(0,3) );
...
//			dR_qz = (dR_qz * currentRotation).eval();
		...
		ddiff_s_tx.template block<3,1>(0,0) = -dt_tx;
		ddiff_s_ty.template block<3,1>(0,0) = -dt_ty;
		ddiff_s_tz.template block<3,1>(0,0) = -dt_tz;
...
template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::GradientFunctor::~GradientFunctor() {

}


template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::GradientFunctor::operator()( const tbb::blocked_range<size_t>& r ) const {
		for( size_t i=r.begin(); i!=r.end(); ++i )
			(*this)((*assocList_)[i]);
	}
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::GradientFunctor::operator()( SurfelAssociation& assoc ) const {
		...
		Eigen::Vector4d pos;
		pos.template block<3,1>(0,0) = assoc.src_->mean_.template block<3,1>(0,0);
		pos(3,0) = 1.f;
		...
		Eigen::Matrix3d cov1_ss;
		Eigen::Matrix3d cov2_ss = assoc.src_->cov_.template block<3,3>(0,0) + cov_ss_add;

		Eigen::Vector3d dstMean;
		Eigen::Vector3d srcMean = assoc.src_->mean_.template block<3,1>(0,0);
		...
		if( interpolate_neighbors_ ) {
			...
			Eigen::Vector3d centerDiff = assoc.n_dst_->getCenterPosition().template block<3,1>(0,0).template cast<double>() - pos_src.template block<3,1>(0,0);
			if( resolution - fabsf(centerDiff(0)) > 0  && resolution - fabsf(centerDiff(1)) > 0  && resolution - fabsf(centerDiff(2)) > 0 ) {
				...
				for( int s = 0; s < 27; s++ ) {
					...
					spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n_dst_n = assoc.n_dst_->neighbors_[s];

					if(!n_dst_n)
						continue;

					typename TMRSMap::Surfel* dst_n = &n_dst_n->value_.surfels_[assoc.dst_idx_];
					if( dst_n->num_points_ < MIN_SURFEL_POINTS )
						continue;

					Eigen::Vector3d centerDiff_n = n_dst_n->getCenterPosition().template block<3,1>(0,0).template cast<double>() - pos_src.template block<3,1>(0,0);
					const double dx = resolution - fabsf(centerDiff_n(0));
					const double dy = resolution - fabsf(centerDiff_n(1));
					const double dz = resolution - fabsf(centerDiff_n(2));
		...
		if( !interpolate_neighbors_ || !in_interpolation_range ) {
			...
			dstMean = assoc.dst_->mean_.template block<3,1>(0,0);
			cov1_ss = assoc.dst_->cov_.template block<3,3>(0,0) + cov_ss_add;
		...
		const Eigen::Vector3d TsrcMean = pos_src.template block<3,1>(0,0);
...
template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::GradientFunctorLM::GradientFunctorLM(SurfelAssociationList* assocList, const Params& params, double tx, double ty, double tz, double qx, double qy, double qz, double qw, bool derivs, bool derivZ, bool interpolate_neighbors) {
...
template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::GradientFunctorLM::~GradientFunctorLM() {

}


template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::GradientFunctorLM::operator()( const tbb::blocked_range<size_t>& r ) const {
		for( size_t i=r.begin(); i!=r.end(); ++i )
			(*this)((*assocList_)[i]);
}
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::GradientFunctorLM::operator()( SurfelAssociation& assoc ) const {
		...
		Eigen::Vector4d pos;
		pos.template block<3,1>(0,0) = srcMean;
		pos(3,0) = 1.f;
...
////		}
//
//		const Eigen::Matrix3d cov1_ss = assoc.dst_->cov_.template block<3,3>(0,0);// + cov_ss_add;
//		const Eigen::Matrix3d cov2_ss = assoc.src_->cov_.template block<3,3>(0,0);// + cov_ss_add;
//
//		const Eigen::Vector3d dstMean = assoc.dst_->mean_.template block<3,1>(0,0);
//		const Eigen::Vector3d srcMean = assoc.src_->mean_.template block<3,1>(0,0);



		const Eigen::Vector3d p_s = pos_src.template block<3,1>(0,0);
		...
		if( derivs_ ) {
			...
			assoc.dh_dx.template block<3,1>(0,0) = dt_tx;
			assoc.dh_dx.template block<3,1>(0,1) = dt_ty;
			assoc.dh_dx.template block<3,1>(0,2) = dt_tz;
//			assoc.df_dx.template block<3,1>(0,3) = dR_qx * srcMean;
//			assoc.df_dx.template block<3,1>(0,4) = dR_qy * srcMean;
//			assoc.df_dx.template block<3,1>(0,5) = dR_qz * srcMean;
			assoc.dh_dx.template block<3,1>(0,3) = dR_qx * pos_src.template block<3,1>(0,0);
			assoc.dh_dx.template block<3,1>(0,4) = dR_qy * pos_src.template block<3,1>(0,0);
			assoc.dh_dx.template block<3,1>(0,5) = dR_qz * pos_src.template block<3,1>(0,0);
            ...
            if( derivZ_ ) {
                    ...
                    const Eigen::Vector3d invcov_ss_ddiff_s_qx = assoc.W * assoc.dh_dx.template block<3,1>(0,3);
                    const Eigen::Vector3d invcov_ss_ddiff_s_qy = assoc.W * assoc.dh_dx.template block<3,1>(0,4);
                    const Eigen::Vector3d invcov_ss_ddiff_s_qz = assoc.W * assoc.dh_dx.template block<3,1>(0,5);
...
//		assert( !isnan(assoc.error) );
	...
	// dh/dx
template <typename TMRSMap>
inline Eigen::Matrix<double, 3, 6> MultiResolutionSurfelRegistration<TMRSMap>::GradientFunctorPointFeature::dh_dx(const Eigen::Vector3d& m,const Eigen::Matrix3d& rot, const Eigen::Vector3d& transl) const {
		...
		tmp(2) += 1 / a2;
		dPI_da.template block<3, 1>(0, 2) = tmp;

		Eigen::Matrix<double, 3, 3> da_dq;
		da_dq.template block<3, 1>(0, 0) = dR_qx * rot * Phi;
		da_dq.template block<3, 1>(0, 1) = dR_qy * rot * Phi;
		da_dq.template block<3, 1>(0, 2) = dR_qz * rot * Phi;

		Eigen::Matrix<double, 3, 6> dh_dx;
		dh_dx.template block<3, 3>(0, 0) = dPI_da;
		dh_dx.template block<3, 3>(0, 3) = dPI_da * da_dq;
...
// dh/dm
template <typename TMRSMap>
inline Eigen::Matrix3d MultiResolutionSurfelRegistration<TMRSMap>::GradientFunctorPointFeature::dh_dm(const Eigen::Vector3d& m, const Eigen::Matrix3d& rot, const Eigen::Vector3d& transl) const {
...
//		double a2 = alpha(2) * alpha(2);
		...
		tmp(1) = -m(1) * inv_m22;
		tmp(2) = -inv_m22;
		da_dm.template block<3, 1>(0, 2) = params_.KInv_ * tmp;

		return dPI_da * da_dm;
}

template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::GradientFunctorPointFeature::GradientFunctorPointFeature(TMRSMap* source,
			TMRSMap* target,
		...
		currentTransform.setIdentity();
		currentTransform.template block<3, 3>(0, 0) = Eigen::Matrix3d(
...
template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::GradientFunctorPointFeature::~GradientFunctorPointFeature() {
}


template <typename TMRSMap>
double MultiResolutionSurfelRegistration<TMRSMap>::preparePointFeatureDerivatives( const Eigen::Matrix<double, 6, 1>& x, double qw, double mahaldist ) {
	double error = 0;

	Eigen::Matrix3d id = Eigen::Matrix3d::Identity();
	Eigen::Vector3d null = Eigen::Vector3d::Zero();

	FeatureAssociationList& fal = featureAssociations_;
	...
	for (typename FeatureAssociationList::iterator it = fal.begin(); it != fal.end(); ++it) {
...
template <typename TMRSMap>
bool MultiResolutionSurfelRegistration<TMRSMap>::registrationErrorFunctionWithFirstDerivative( const Eigen::Matrix< double, 6, 1 >& x, double& f, Eigen::Matrix< double, 6, 1 >& df_dx, SurfelAssociationList& surfelAssociations ) {
	...
	double numMatches = 0;
	for( typename SurfelAssociationList::iterator it = surfelAssociations.begin(); it != surfelAssociations.end(); ++it ) {
...
template <typename TMRSMap>
bool MultiResolutionSurfelRegistration<TMRSMap>::registrationErrorFunctionWithFirstAndSecondDerivative( const Eigen::Matrix< double, 6, 1 >& x, bool relativeDerivative, double& f, Eigen::Matrix< double, 6, 1 >& df_dx, Eigen::Matrix< double, 6, 6 >& d2f_dx2, SurfelAssociationList& surfelAssociations ) {
...
//			p1.z = it->dst_->mean_[2];
			...
			Eigen::Vector4d pos;
			pos.template block<3,1>(0,0) = pos2.template block<3,1>(0,0).cast<double>();
//			pos.template block<3,1>(0,0) = it->src_->mean_.template block<3,1>(0,0);
			pos(3,0) = 1.f;
...
template <typename TMRSMap>
bool MultiResolutionSurfelRegistration<TMRSMap>::registrationErrorFunctionLM( const Eigen::Matrix< double, 6, 1 >& x, double& f, SurfelAssociationList& surfelAssociations, FeatureAssociationList& featureAssociations, double mahaldist ) {
	...
	if (params_.registerFeatures_) {
		...
		transform.template block<3, 1>(0, 3) = trnsl;

		for (typename FeatureAssociationList::iterator it = featureAssociations.begin(); it != featureAssociations.end(); ++it) {

//			if( !it->match )
//				continue;
			it->match = 1;

			Eigen::Vector3d src = source_->features_[it->src_idx_].invzpos_.template block<3, 1>(0,0);
			Eigen::Vector3d dst = target_->features_[it->dst_idx_].invzpos_.template block<3, 1>(0,0);

			Eigen::Vector3d lm = it->landmark_pos;

			double src_error = (src - phiInv(rot * lm + trnsl)).transpose() * source_->features_[it->src_idx_].invzcov_.template block<3, 3>(0,0).inverse() * (src - phiInv(rot * lm + trnsl));
			double dst_error = (dst - phiInv(lm)).transpose() * target_->features_[it->dst_idx_].invzcov_.template block<3, 3>(0,0).inverse() * (dst - phiInv(lm));
...
template <typename TMRSMap>
bool MultiResolutionSurfelRegistration<TMRSMap>::registrationSurfelErrorFunctionWithFirstAndSecondDerivativeLM( const Eigen::Matrix< double, 6, 1 >& x, double& f, Eigen::Matrix< double, 6, 1 >& df, Eigen::Matrix< double, 6, 6 >& d2f, SurfelAssociationList& surfelAssociations ) {
...
template <typename TMRSMap>
bool MultiResolutionSurfelRegistration<TMRSMap>::estimateTransformationNewton( Eigen::Matrix4d& transform, int coarseToFineIterations, int fineIterations ) {
	...
	Eigen::Quaterniond q( currentTransform.template block<3,3>(0,0) );
	...
	const double qy = x( 4 );
	const double qz = x( 5 );
	transform.template block<3,3>(0,0) = Eigen::Matrix3d( Eigen::Quaterniond( lastWSign_*sqrt(1.0-qx*qx-qy*qy-qz*qz), qx, qy, qz ) );
	transform(0,3) = x( 0 );
	transform(1,3) = x( 1 );
	transform(2,3) = x( 2 );
	...
	while( iter < maxIterations ) {
		...
		if( fabsf( d2f.determinant() ) > std::numeric_limits<double>::epsilon() ) {
			...
			deltaTransform.template block<3,3>(0,0) = Eigen::Matrix3d( Eigen::Quaterniond( qw, qx, qy, qz ) );
			...
			Eigen::Quaterniond q_new( newTransform.template block<3,3>(0,0) );
			x( 3 ) = q_new.x();
			x( 4 ) = q_new.y();
			x( 5 ) = q_new.z();
		...
		transform.setIdentity();
		transform.template block<3,3>(0,0) = Eigen::Matrix3d( Eigen::Quaterniond( qw, qx, qy, qz ) );
		transform(0,3) = x( 0 );
		transform(1,3) = x( 1 );
		transform(2,3) = x( 2 );
...
template <typename TMRSMap>
bool MultiResolutionSurfelRegistration<TMRSMap>::estimateTransformationLevenbergMarquardt( Eigen::Matrix4d& transform, int maxIterations, SurfelAssociationList* surfelAssociationsArg, bool knownAssociations, bool interpolate ) {
...
template <typename TMRSMap>
bool MultiResolutionSurfelRegistration<TMRSMap>::estimateTransformationGaussNewton( Eigen::Matrix4d& transform, int maxIterations, SurfelAssociationList* surfelAssociationsArg, bool knownAssociations, bool interpolate ) {
...
template <typename TMRSMap>
bool MultiResolutionSurfelRegistration<TMRSMap>::estimateTransformationLevenbergMarquardtPF( Eigen::Matrix4d& transform, int maxIterations, double featureAssocMahalDist, double minDelta, bool resetFeatures, double& mu, double& nu ) {
	...
	if( resetFeatures ) {
		for (typename FeatureAssociationList::iterator it = featureAssociations_.begin(); it != featureAssociations_.end(); ++it) {
			it->landmark_pos = target_->features_[it->dst_idx_].pos_.template block<3, 1>(0,0);
		}
	}
	...
	Eigen::Matrix<double, 6, 1> poseOFdf	= Eigen::Matrix<double, 6, 1>::Zero(); // df.template block<6,1>(0,0), df = J^T * Sigma * diff
...
//	double nu = 2;
	...
	while (iter < maxIterations) {
		...
		if( params_.debugFeatures_ ) {
			...
			for(typename FeatureAssociationList::iterator it = featureAssociations_.begin(); it != featureAssociations_.end(); ++it ) {
				...
				Eigen::Vector3d dst = target_->features_[ it->dst_idx_ ].invzpos_.template block<3,1>(0,0);
				Eigen::Vector3d src = source_->features_[ it->src_idx_ ].invzpos_.template block<3,1>(0,0);
		//			Eigen::Vector3d src = h( dst, backTrnsf.template block<3,3>(0,0), backTrnsf.template block<3,1>(0,3));
	...
	//			cv::line( frameA, srcPoint, dstPoint, rot, 1, 0, 0 );
				...
				// LM-Schätzungen
				Eigen::Vector3d pixA = phiInv( transform.template block<3,3>(0,0) * it->landmark_pos + transform.template block<3,1>(0,3) );
				Eigen::Vector3d pixB = phiInv( it->landmark_pos );
				cv::Point srcLMPoint( pixA(0) , pixA(1) );
				cv::Point dstLMPoint( pixB(0) , pixB(1) );

				if ( (pixA - src).template block<2,1>(0,0).norm() > 10 )
				{
				...
				if ( (pixB - dst).template block<2,1>(0,0).norm() > 10 )
				{
...
//		for (MRCSRFAL::iterator it = featureAssociations_.begin(); it != featureAssociations_.end(); ++it)
//		{
//			FeatureAssociation assoc( it->src_idx_, it->dst_idx_ );
//
//			if (it->match == 0) {
...
//		}
		...
		if (!retVal2)
		{
			std::cout << "2nd ErrorFunction for AreNo and typename TMRSMap::Surfel failed\n";
			return false;
		}
...
//			}
		...
		transform.setIdentity();
		transform.template block<3, 3>(0, 0) = Eigen::Matrix3d(
				Eigen::Quaterniond(qw, qx, qy, qz));
		transform(0, 3) = x(0);
		transform(1, 3) = x(1);
		transform(2, 3) = x(2);
...
template <typename TMRSMap>
bool MultiResolutionSurfelRegistration<TMRSMap>::estimateTransformationGaussNewtonPF( Eigen::Matrix4d& transform, int maxIterations, double featureAssocMahalDist, double minDelta, bool resetFeatures ) {
...
template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::MatchLogLikelihoodFunctor::MatchLogLikelihoodFunctor() {

}

template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::MatchLogLikelihoodFunctor::MatchLogLikelihoodFunctor( NodeLogLikelihoodList* nodes, Params params, TMRSMap* source, TMRSMap* target, const Eigen::Matrix4d& transform, const Eigen::Matrix4d& lastTransform, const Eigen::Matrix< double, 6, 6 >& pcov, double delta_t, bool addPoseCov ) {
		nodes_ = nodes;
		params_ = params;
		source_ = source;
		target_ = target;
		...
		lastRotation = lastTransform.template block<3,3>(0,0);
		...
		currentRotation = Eigen::Matrix3d( targetToSourceTransform.template block<3,3>(0,0) );
		currentRotationT = currentRotation.transpose();
		currentTranslation = Eigen::Vector3d( targetToSourceTransform.template block<3,1>(0,3) );
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::MatchLogLikelihoodFunctor::precomputeCovAdd( TMRSMap* target ) {
...
template <typename TMRSMap>
Eigen::Matrix< double, 3, 6 > MultiResolutionSurfelRegistration<TMRSMap>::MatchLogLikelihoodFunctor::diff_jac_for_pose( const Eigen::Vector3d& position ) const {

		// Jacobian of diff mu_tgt - T(x) mu_src is: - dT(x) mu_src in R^(3 x 6)

		Eigen::Matrix< double, 3, 6 > jac = Eigen::Matrix< double, 3, 6 >::Zero();

		jac.template block<3,1>(0,0) = (lastTransform * Eigen::Vector4d::Unit( 0 )).template block<3,1>(0,0);
		jac.template block<3,1>(0,1) = (lastTransform * Eigen::Vector4d::Unit( 1 )).template block<3,1>(0,0);
		jac.template block<3,1>(0,2) = (lastTransform * Eigen::Vector4d::Unit( 2 )).template block<3,1>(0,0);
		jac.template block<3,1>(0,3) = (lastTransform * Eigen::Vector4d( 0, -position(2), position(1), 0 )).template block<3,1>(0,0);
		jac.template block<3,1>(0,4) = (lastTransform * Eigen::Vector4d( position(2), -position(0), 0, 0 )).template block<3,1>(0,0);
		jac.template block<3,1>(0,5) = (lastTransform * Eigen::Vector4d( -position(1), position(0), 0, 0 )).template block<3,1>(0,0);
...
//		jac( 2, 2 ) = -1;
//
//		jac( 1, 3 ) = +rotpos(2);//(transform_.template block<1,3>(2,0) * position)(0);
//		jac( 2, 3 ) = -rotpos(1);//(transform_.template block<1,3>(1,0) * position)(0);
//
//		jac( 0, 4 ) = -rotpos(2);//(transform_.template block<1,3>(2,0) * position)(0);
//		jac( 2, 4 ) = +rotpos(0);//(transform_.template block<1,3>(0,0) * position)(0);
//
//		jac( 0, 5 ) = +rotpos(1);//(transform_.template block<1,3>(1,0) * position)(0);
//		jac( 1, 5 ) = -rotpos(0);//(transform_.template block<1,3>(0,0) * position)(0);
//
//		return jac;
...
//template <typename TMRSMap>
//	Eigen::Matrix< double, 1, 3 > MultiResolutionSurfelRegistration<TMRSMap>::MatchLogLikelihoodFunctor::diff_normals_jac_for_angvel( const Eigen::Vector3d& normal1, const Eigen::Vector3d& normal2 ) const {
//
...
template <typename TMRSMap>
double MultiResolutionSurfelRegistration<TMRSMap>::MatchLogLikelihoodFunctor::normalCovFromPoseCov( const Eigen::Vector3d& normal1, const Eigen::Vector3d& normal2, const Eigen::Matrix3d& poseCov ) const {
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::MatchLogLikelihoodFunctor::operator()( const tbb::blocked_range<size_t>& r ) const {
		for( size_t i=r.begin(); i!=r.end(); ++i )
			(*this)((*nodes_)[i]);
	}
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::MatchLogLikelihoodFunctor::operator()( NodeLogLikelihood& node ) const {

		spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n = node.n_;

		double sumLogLikelihood = 0.0;

		Eigen::Vector4d npos = n->getPosition().template cast<double>();
		npos(3) = 1;
		...
		std::vector< spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* > nodes;
		nodes.reserve(50);
...
//		for( unsigned int i = 0; i < typename TMRSMap::NodeValue::num_surfels_; i++ ) {

			node.surfelassocs_[i].loglikelihood_ = 0.0;
			node.surfelassocs_[i].n_dst_ = NULL;
			node.surfelassocs_[i].dst_ = NULL;

			typename TMRSMap::Surfel* targetSurfel = &n->value_.surfels_[i];
			...
			Eigen::Vector4d pos;
			pos.template block<3,1>(0,0) = targetSurfel->mean_.template block<3,1>(0,0);
			pos(3,0) = 1.f;

			Eigen::Vector4d dir;
			dir.template block<3,1>(0,0) = targetSurfel->initial_view_dir_;
			...
			Eigen::Matrix3d cov2 = targetSurfel->cov_.template block<3,3>(0,0);
			cov2 += cov_add;
			...
			if( add_pose_cov_ ) {
				const Eigen::Vector3d pos3 = pos.template block<3,1>(0,0);
			...
			if( params_.match_likelihood_use_color_ ) {
				nomatch_loglikelihood += -0.5 * log( 8.0 * M_PI * M_PI * M_PI * (targetSurfel->cov_.template block<3,3>(3,3) + 0.0001 * Eigen::Matrix3d::Identity()).determinant() ) + mahaldist;
			}
			...
			if( !targetSurfel->seenThrough_ ) {
				...
				for( typename std::vector< spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* >::iterator it = nodes.begin(); it != nodes.end(); ++it ) {

					spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n_src = *it;
					...
					for( unsigned int k = 0; k < TMRSMap::NodeValue::num_surfels_; k++ ) {
						...
						const double dist = dir_match_src.template block<3,1>(0,0).dot( n_src->value_.surfels_[k].initial_view_dir_ );
					...
					Eigen::Vector3d diff_pos = bestMatchSurfel->mean_.template block<3,1>(0,0) - pos_match_src.template block<3,1>(0,0);


					Eigen::Matrix3d cov1 = bestMatchSurfel->cov_.template block<3,3>(0,0);
					cov1 += cov_add;
					...
					Eigen::Vector3d diff_col = bestMatchSurfel->mean_.template block<3,1>(3,0) - targetSurfel->mean_.template block<3,1>(3,0);
					...
					if( params_.match_likelihood_use_color_ ) {
						const Eigen::Matrix3d cov_cc = bestMatchSurfel->cov_.template block<3,3>(3,3) + targetSurfel->cov_.template block<3,3>(3,3) + 0.0001 * Eigen::Matrix3d::Identity();
					...
					if( params_.match_likelihood_use_normals_ ) {
						...
						Eigen::Vector4d normal_src;
						normal_src.template block<3,1>(0,0) = targetSurfel->normal_;
						normal_src(3,0) = 0.0;
						...
						if( add_pose_cov_ ) {
		//					Eigen::Matrix< double, 1, 3 > jac_normals = diff_normals_jac_for_angvel( bestMatchSurfel->normal_, normal_src.template block<3,1>(0,0) );
		//					normalStdMod += (jac_normals * pcov_.template block<3,3>(3,3) * jac_normals.transpose())(0);
							normalStdMod += normalCovFromPoseCov( bestMatchSurfel->normal_, normal_src.template block<3,1>(0,0), pcov_.template block<3,3>(3,3) );

		//					std::cout << "normal std added: " << normalStdMod-normalStd << "\n";
						}

						double normalError = acos( normal_src.template block<3,1>(0,0).dot( bestMatchSurfel->normal_ ) );
...
template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::MatchLogLikelihoodKnownAssociationsFunctor::MatchLogLikelihoodKnownAssociationsFunctor( NodeLogLikelihoodList* associations, Params params, TMRSMap* source, TMRSMap* target, const Eigen::Matrix4d& transform, const Eigen::Matrix4d& lastTransform, const Eigen::Matrix< double, 6, 6 >& pcov, double delta_t, bool addPoseCov ) {
		this->nodes_ = associations;
		this->params_ = params;
		this->source_ = source;
		this->target_ = target;
		this->transform_ = transform;
		this->add_pose_cov_ = addPoseCov;
		this->pcov_ = pcov;
		this->delta_t_ = delta_t;
		this->lastTransform = lastTransform;
		this->lastRotation = lastTransform.template block<3,3>(0,0);

		this->normalStd = 0.125*M_PI;
		this->normalMinLogLikelihood = -0.5 * log( 2.0 * M_PI * this->normalStd ) - 4.5;
		this->normalMinLogLikelihoodSeenThrough = -0.5 * log( 2.0 * M_PI * this->normalStd ) - 8.0;
		...
		this->targetToSourceTransform = this->transform_;
		this->currentRotation = Eigen::Matrix3d( this->targetToSourceTransform.template block<3,3>(0,0) );
		this->currentRotationT = this->currentRotation.transpose();
		this->currentTranslation = Eigen::Vector3d( this->targetToSourceTransform.template block<3,1>(0,3) );
...
template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::MatchLogLikelihoodKnownAssociationsFunctor::~MatchLogLikelihoodKnownAssociationsFunctor() {

}


template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::MatchLogLikelihoodKnownAssociationsFunctor::operator()( const tbb::blocked_range<size_t>& r ) const {
		for( size_t i=r.begin(); i!=r.end(); ++i )
			(*this)((*(this->nodes_))[i]);
}
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::MatchLogLikelihoodKnownAssociationsFunctor::operator()( NodeLogLikelihood& node ) const {

		double sumLogLikelihood = 0.0;
		spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n = node.n_;

		const Eigen::Matrix3d& cov_add = this->cov_add_[n->depth_];
...
//		for( unsigned int i = 0; i < typename TMRSMap::NodeValue::num_surfels_; i++ ) {
			...
			if( node.surfelassocs_[i].dst_ ) {
				...
				Eigen::Vector4d pos_match_src = this->targetToSourceTransform * pos;

				Eigen::Matrix3d cov2 = targetSurfel->cov_.template block<3,3>(0,0);
				cov2 += cov_add;

				Eigen::Matrix3d cov2_RT = cov2 * this->currentRotationT;
				Eigen::Matrix3d cov2_rotated = (this->currentRotation * cov2_RT).eval();

				if( this->add_pose_cov_ ) {
					const Eigen::Vector3d pos3 = pos.template block<3,1>(0,0);
					const Eigen::Matrix< double, 3, 6 > jac = this->diff_jac_for_pose( pos3 );
					cov2_rotated += jac * this->pcov_ * jac.transpose();
				}

				typename TMRSMap::Surfel* sceneSurfel = node.surfelassocs_[i].dst_;

				Eigen::Vector3d diff_pos = sceneSurfel->mean_.template block<3,1>(0,0) - pos_match_src.template block <3,1>(0,0);

				Eigen::Matrix3d cov1 = sceneSurfel->cov_.template block<3,3>(0,0);
				cov1 += cov_add;
				...
				Eigen::Vector3d diff_col = sceneSurfel->mean_.template block<3,1>(3,0) - targetSurfel->mean_.template block<3,1>(3,0);
				if( fabs(diff_col(0)) < params_.luminance_damp_diff_ )
					diff_col(0) = 0;
				if( fabs(diff_col(1)) < params_.color_damp_diff_ )
					diff_col(1) = 0;
				...
				if( params_.match_likelihood_use_color_ ) {
					const Eigen::Matrix3d cov_cc = sceneSurfel->cov_.template block<3,3>(3,3) + targetSurfel->cov_.template block<3,3>(3,3) + 0.0001 * Eigen::Matrix3d::Identity();
					const Eigen::Matrix3d invcov_cc = cov_cc.inverse();
					double color_exponent = -0.5 * diff_col.dot( (invcov_cc * diff_col ).eval() );
					if( color_exponent < -6.0 )
						color_exponent = -13.5;
				...
				if( params_.match_likelihood_use_normals_ ) {
					...
					normal_src.template block<3,1>(0,0) = targetSurfel->normal_;
					normal_src(3,0) = 0.0;
					normal_src = (this->targetToSourceTransform * normal_src).eval();


					double normalStdMod = this->normalStd;

					if( this->add_pose_cov_ ) {
	//					Eigen::Matrix< double, 1, 3 > jac_normals = diff_normals_jac_for_angvel( bestMatchSurfel->normal_, normal_src.template block<3,1>(0,0) );
	//					normalStdMod += (jac_normals * pcov_.template block<3,3>(3,3) * jac_normals.transpose())(0);
						normalStdMod += normalCovFromPoseCov( sceneSurfel->normal_, normal_src.template block<3,1>(0,0), this->pcov_.template block<3,3>(3,3) );

	//					std::cout << "normal std added: " << normalStdMod-normalStd << "\n";
					}

					double normalError = acos( normal_src.template block<3,1>(0,0).dot( sceneSurfel->normal_ ) );
					double normalExponent = -0.5 * normalError * normalError / ( normalStdMod*normalStdMod );
					if( normalExponent < -2.0 )
						normalExponent = -4.5;
					double normalLogLikelihood = -0.5 * log( 2.0 * M_PI * normalStdMod ) + normalExponent;
...
template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::ShootThroughFunctor::ShootThroughFunctor( NodeLogLikelihoodList* nodes, Params params, TMRSMap* source, TMRSMap* target, const Eigen::Matrix4d& transform ) {
		nodes_ = nodes;
		params_ = params;
		source_ = source;
		target_ = target;
		...
		camera_pos_ = transform_inv_.template block<3,1>(0,3);

}

template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::ShootThroughFunctor::~ShootThroughFunctor() {

}


template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::ShootThroughFunctor::operator()( const tbb::blocked_range<size_t>& r ) const {
		for( size_t i=r.begin(); i!=r.end(); ++i )
			(*this)((*nodes_)[i]);
}
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::ShootThroughFunctor::operator()( NodeLogLikelihood& node ) const {

		spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n = node.n_;

		Eigen::Vector3d viewDirection = n->getCenterPosition().template cast<double>().template block<3,1>(0,0) - camera_pos_;
		const typename TMRSMap::Surfel* targetSurfel = n->value_.getSurfel( viewDirection );
...
template <typename TMRSMap>
double MultiResolutionSurfelRegistration<TMRSMap>::matchLogLikelihood( TMRSMap& source, TMRSMap& target, Eigen::Matrix4d& transform, const Eigen::Matrix4d& lastTransform, const Eigen::Matrix< double, 6, 6 >& pcov, double delta_t, bool addPoseCov ) {
	...
	// check for scene occlusions by the model
	{
		...
		for( int d = params_.model_visibility_max_depth_; d >= 0; d-- ) {
			...
			for( unsigned int i = 0; i < sourceSamplingMap[d].size(); i++ ) {
				spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n = sourceSamplingMap[d][i];
...
template <typename TMRSMap>
double MultiResolutionSurfelRegistration<TMRSMap>::matchLogLikelihoodKnownAssociations( TMRSMap& source, TMRSMap& target, Eigen::Matrix4d& transform, NodeLogLikelihoodList& associations, const Eigen::Matrix4d& lastTransform, const Eigen::Matrix< double, 6, 6 >& pcov, double delta_t, bool addPoseCov ) {
...
//
//template <typename TMRSMap>
//NodeLogLikelihoodList MultiResolutionSurfelRegistration<TMRSMap>::precalculateNomatchLogLikelihoodKnownAssociations( TMRSMap& source, TMRSMap& target, Eigen::Matrix4d& transform, SurfelAssociationList& surfelAssociations, const Eigen::Matrix4d& lastTransform, const Eigen::Matrix< double, 6, 6 >& pcov, double delta_t, bool addPoseCov ) {
//
//	double sumLogLikelihood = 0.0;
//
//	targetSamplingMap_ = algorithm::downsampleVectorOcTree(*target.octree_, false, target.octree_->max_depth_);
...
//
//template <typename TMRSMap>
//void MultiResolutionSurfelRegistration<TMRSMap>::nomatchLogLikelihoodKnownAssociationsResetAssocs( const NodeLogLikelihoodList& nodes ) {
//
//	for( unsigned int i = 0; i < nodes.size(); i++ ) {
...
//
//template <typename TMRSMap>
//double MultiResolutionSurfelRegistration<TMRSMap>::nomatchLogLikelihoodKnownAssociationsPreCalc( const NodeLogLikelihoodList& nodes ) {
//
//	double sumLogLikelihood = 0.0;
//
//	for( unsigned int i = 0; i < nodes.size(); i++ ) {
...
//
//template <typename TMRSMap>
//double MultiResolutionSurfelRegistration<TMRSMap>::nomatchLogLikelihoodKnownAssociations( TMRSMap& source, TMRSMap& target, Eigen::Matrix4d& transform, SurfelAssociationList& surfelAssociations, const Eigen::Matrix4d& lastTransform, const Eigen::Matrix< double, 6, 6 >& pcov, double delta_t, bool addPoseCov ) {
//
//	double sumLogLikelihood = 0.0;
//
//	targetSamplingMap_ = algorithm::downsampleVectorOcTree(*target.octree_, false, target.octree_->max_depth_);
...
template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::SelfMatchLogLikelihoodFunctor::SelfMatchLogLikelihoodFunctor( NodeLogLikelihoodList* nodes, Params params, TMRSMap* target ) {
		nodes_ = nodes;
		params_ = params;
		target_ = target;
...
template <typename TMRSMap>
MultiResolutionSurfelRegistration<TMRSMap>::SelfMatchLogLikelihoodFunctor::~SelfMatchLogLikelihoodFunctor() {
}


template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::SelfMatchLogLikelihoodFunctor::operator()( const tbb::blocked_range<size_t>& r ) const {
		for( size_t i=r.begin(); i!=r.end(); ++i )
			(*this)((*nodes_)[i]);
}
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::SelfMatchLogLikelihoodFunctor::operator()( NodeLogLikelihood& node ) const {
		...
		for( unsigned int i = 0; i < TMRSMap::NodeValue::num_surfels_; i++ ) {
			...
			typename TMRSMap::Surfel* modelSurfel = &n->value_.surfels_[i];
			...
			// precalculate log likelihood when surfel is not matched in the scene
			Eigen::Matrix3d cov2 = modelSurfel->cov_.template block<3,3>(0,0);
			cov2 += cov_add;

			double nomatch_loglikelihood = -0.5 * log( 8.0 * M_PI * M_PI * M_PI * cov2.determinant() ) - 24.0;

			if( params_.match_likelihood_use_color_ ) {
				nomatch_loglikelihood += -0.5 * log( 8.0 * M_PI * M_PI * M_PI * modelSurfel->cov_.template block<3,3>(3,3).determinant() ) - 24.0;
			}
...
//			Eigen::Matrix3d invcov = cov.inverse().eval();
			...
			if( params_.match_likelihood_use_color_ ) {
				const Eigen::Matrix3d cov_cc = 2.0 * modelSurfel->cov_.template block<3,3>(3,3) + 0.01 * Eigen::Matrix3d::Identity();
...
template <typename TMRSMap>
double MultiResolutionSurfelRegistration<TMRSMap>::selfMatchLogLikelihood( TMRSMap& target ) {
...
template <typename TMRSMap>
bool MultiResolutionSurfelRegistration<TMRSMap>::estimatePoseCovariance( Eigen::Matrix< double, 6, 6 >& poseCov, TMRSMap& source, TMRSMap& target, Eigen::Matrix4d& transform, float startResolution, float stopResolution ) {
	...
	algorithm::OcTreeSamplingVectorMap<float, typename TMRSMap::NodeValue> targetSamplingMap = algorithm::downsampleVectorOcTree(*target.octree_, false, target.octree_->max_depth_);

	double sumWeight = 0.0;

	Eigen::Quaterniond q( transform.template block<3,3>(0,0) );
...
template <typename TMRSMap>
bool MultiResolutionSurfelRegistration<TMRSMap>::estimatePoseCovarianceLM( Eigen::Matrix< double, 6, 6 >& poseCov, TMRSMap& source, TMRSMap& target, Eigen::Matrix4d& transform, float startResolution, float stopResolution, SurfelAssociationList* surfelAssociationsArg, bool knownAssociations ) {
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::improvedProposalMatchLogLikelihoodKnownAssociations( TMRSMap& source, TMRSMap& target,
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::improvedProposalMatchLogLikelihood( TMRSMap& source, TMRSMap& target,
...
template <typename TMRSMap>
void MultiResolutionSurfelRegistration<TMRSMap>::getAssociations( SurfelAssociationList& surfelAssociations, TMRSMap& source, TMRSMap& target,
...
template <typename TMRSMap>
bool MultiResolutionSurfelRegistration<TMRSMap>::registerPose( SurfelAssociationList& surfelAssociations, TMRSMap& source, TMRSMap& target,
    ...
    if( regularizeRegistration ) {
		setPriorPose( true, pose.mean_.asVector().template block<6,1>(0,0), registrationPriorPoseCov );
    }
    else
...
template <typename TMRSMap>
bool MultiResolutionSurfelRegistration<TMRSMap>::registerPoseKnownAssociations( SurfelAssociationList& surfelAssociations, TMRSMap& source, TMRSMap& target,
                                                                       Geometry::PoseWithCovariance & pose, Geometry::PoseWithCovariance & poseOut,
                                                                       pcl::PointCloud< pcl::PointXYZRGB >::Ptr & corrSrc,
                                                                       pcl::PointCloud< pcl::PointXYZRGB >::Ptr & corrTgt,
                                                                       const int numRegistrationStepsCoarse, const int numRegistrationStepsFine,
...
template <typename TMRSMap>
bool mrsmap::pointOccluded( const Eigen::Vector4f& p, const TMRSMap& target, double z_similarity_factor ) {
	...
	if( !target.imageAllocator_->node_set_.empty() ) {
		...
		unsigned int idx = py * 640 + px;
		const spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n = target.imageAllocator_->node_image_[idx];
		if( n ) {
			double z_dist = std::max( 0.f, p(2) - n->getCenterPosition()(2) );
			if( z_dist > fabsf( z_similarity_factor * p(2) ) )
				return true;
...
template <typename TMRSMap>
bool mrsmap::pointSeenThrough( const Eigen::Vector4f& p, const TMRSMap& target, double z_similarity_factor, bool markSeenThrough) {
	...
	if( !target.imageAllocator_->node_set_.empty() ) {
		...
		if( n ) {
			...
			if( z_dist > fabsf( z_similarity_factor * p(2) ) ) {
				if( markSeenThrough ) {
					spatialaggregate::OcTreeNode< float, typename TMRSMap::NodeValue >* n2 = n;
					while( n2 != NULL ) {
//						for( unsigned int i = 0; i < typename TMRSMap::NodeValue::num_surfels_; i++ )
